...
function createPopup(text, options = {}) {
  ...
  popup.addEventListener('click', (ev) => {
    // 拖拽过程中不触发点击消失；吸入中也不处理
    if (popup.classList.contains('dragging') || popup.classList.contains('absorbed')) return;
    // 防止拖拽结束的“尾随点击”误触（250ms 内忽略一次点击）
    const now = performance.now();
    if (popup.__lastDragEndAt && (now - popup.__lastDragEndAt) < 250) return;
    dismissPopup(popup);
  });
}
...
function enableDrag(popup) {
  ...
  function onPointerDown(e) {
    ...
    function onPointerUp(ev) {
      ...
      // 记录拖拽结束时间，仅在发生位移时记录（用于抑制尾随点击）
      popup.__dragActive = false;
      if (popup.__didDrag) popup.__lastDragEndAt = performance.now();
      popup.__didDrag = false;
...
// 立即消失：快速缩小并淡出当前弹窗
function dismissPopup(popup, opts = {}) {
  const { duration = 0.22, scale = 0.85 } = opts;
  popup.classList.add('dismissed');
  popup.classList.remove('dragging');
  popup.classList.remove('over-bag');
  handbag.classList.remove('bag-active');
  popup.style.willChange = 'transform, opacity';
  popup.style.transition = `transform ${duration}s ease-out, opacity ${duration}s ease-out`;
...
handbag.addEventListener('click', () => {
  ...
  for (const p of all) {
    ...
    // 标记拖拽状态，用于避免拖拽结束的尾随点击触发消失
    popup.__dragActive = true;
    popup.__didDrag = false;